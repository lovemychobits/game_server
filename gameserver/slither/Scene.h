...
namespace slither {
	...
	public:
		...
		Snake* GetSnakeByPlayerId(uint64_t uPlayerId);
		Vector2D GenSnakeBornPos();																	// 生成蛇的初始坐标

		set<uint32_t> GetInViewGrids(Snake* pSnake, SnakeViewPoint& viewPoint);						// 获取格子列表，根据蛇本身的视野来决定
		void GetInViewGrids(Snake* pSnake, set<uint32_t>& gridSet);
		GameRoom* GetGameRoom() {
			return m_pGameRoom;
		}

		uint32_t GetGameEndLastRank() {
			return m_uGameEndLastRank;
		}
	...
	protected:
		void SimulateSnakes();																		// 模拟所有蛇的运动

		inline uint32_t GetGridIndex(const Vector2D& pos)											// 获取坐标点所在的格子
		{
			uint32_t x = (uint32_t)slither_min((pos.x), (m_fSceneLength - 1.0f));
			uint32_t y = (uint32_t)slither_min((pos.y), (m_fSceneLength - 1.0f));

			uint32_t uIndex = m_fastIndexArray[y][x];												// y是垂直方向，x是水平方向
			return uIndex;
		}

		void GenFoods(uint32_t uNum);																// 生成食物
		void GenFoods(float fRangeScale, uint32_t uNum);											// 在指定范围内(以中心点)刷食物
		...
		int32_t GetBreakUpFoodSize(Snake* pSnake);													// 获取分解可以得到的食物数量
	...
	protected:
		...
		void BroadcastRankingList(bool bGameEnd = false);											// 广播排行榜信息
	...
	protected:
		...
		map<uint64_t, Snake*> m_playerToSnakeMap;					// 玩家ID对应蛇map
		...
		boost::posix_time::ptime m_lastBroadcastRankTime;			// 最后一次通知排名时间
		uint32_t m_uGameEndLastRank;								// 游戏结束的时候，最后的排名
		uint32_t** m_fastIndexArray;								// 通过点的x，y坐标迅速查找所在格子
	};
}
